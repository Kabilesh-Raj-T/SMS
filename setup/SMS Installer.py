...
mycursor.execute('create database sms_database;')
mycursor.execute('use sms_database')
mycursor.execute('create table product_database(ID varchar(20) primary key,NAME varchar(25),SELLING_PRICE decimal(10,2),COST_PRICE decimal(10,2),BRAND varchar(20),QUANTITY int(11),ITEMS_SOLD int(11));')
mycursor.execute('create table customer_database(ID varchar(20) primary key,NAME varchar(25),CONTACT_NUMBER bigint,GENDER varchar(15),EMAIL varchar(40),PURCHASES mediumtext);')
mycursor.execute('create table employee_database(ID varchar(11) primary key,NAME varchar(25),CONTACT_NUMBER bigint,GENDER varchar(15),POSITION varchar(50),SALARY decimal(10,2));')
...
sq = 'insert into product_database values("{}","{}",{},{},"{}",{},{})'
for i in range(len(a)):
    mycursor.execute(sq.format(a[i],b[i],c[i],g[i],d[i],e[i],f[i]))
mydb.commit()
...
sqa = 'insert into employee_database values("{}","{}",{},"{}","{}",{})'
for ia in range(len(aa)):
    mycursor.execute(sqa.format(aa[ia],ba[ia],ca[ia],da[ia],ea[ia],fa[ia]))
mydb.commit()
...
sqa = 'insert into customer_database values("{}","{}",{},"{}","{}","{}")'
for ib in range(len(ab)):
    mycursor.execute(sqa.format(ab[ib],bb[ib],cb[ib],dib[ib],eb[ib],fb[ib]))
mydb.commit()
