...
def is_already_installed(cnx):
    """Check if product_database table exists → if yes, assume installed."""
    try:
        cursor = cnx.cursor()
        cursor.execute("SHOW TABLES LIKE 'product_database';")
        exists = cursor.fetchone() is not None
        cursor.close()
        return exists
    except Exception as e:
        print("Error checking installation:", e)
        return False
...
try:
    ...
    # Check installation
    # --------------------------
    if is_already_installed(cnx):
        print("⚡ Database already installed. Skipping installer.")
    else:
        ...
        cursor.executemany(
            'INSERT INTO sales_history (product_id, quantity, sale_date) VALUES (%s,%s,%s,%s);',
            sales
        )

        cnx.commit()
        print("🎉 All tables and sample data inserted successfully!")
